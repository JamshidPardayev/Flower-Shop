      ...
      >
        <Box sx={style}>
          <div className='flex justify-end'>
            <FiX onClick={handleModalClose} className='text-[25px] text-[#46A358] hover:text-green-700 cursor-pointer duration-300' />
          </div>
          <Typography id="modal-title" variant="h6" className='flex justify-center gap-x-4'>
            <button onClick={() => setActiveTab('login')} className={`font-bold ${activeTab === 'login' ? 'text-[#46a358]' : ''}`}>Login</button>
            <button onClick={() => setActiveTab('register')} className={`font-bold ${activeTab === 'register' ? 'text-[#46a358]' : ''}`}>Register</button>
          </Typography>
